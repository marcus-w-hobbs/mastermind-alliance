...
export async function sseAgentSSE(req: NextRequest): Promise<Response> {
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (featureFlags.enableAgentMem0) {
        try {
          const userMemoryMessage: Mem0Message = {
            role: "user", 
            content: message.content
          };
          await addMemory(featureFlags.agentMem0UserId, [userMemoryMessage]);
        } catch (error) {
      ...
      if (featureFlags.enableAgentMem0) {
        try {
          ...
          if (!lastUserMessage) {
            ...
            const memoriesStr = await searchMemories(featureFlags.agentMem0UserId, lastUserMessage.content);
            if (memoriesStr) {
        ...
        try {
          ...
          if (featureFlags.enableAgentMem0) {
            try {
              ...
              await addMemory(featureFlags.agentMem0UserId, [assistantMem0Message]);
            } catch (error) {
              console.error("Error adding to memory:", error);
            }
          }
    ...
    return new Response(`SSE Agent error: ${error instanceof Error ? error.message : String(error)}`, { status: 500 });
  }
} 